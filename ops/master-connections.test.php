...
try {
    ...
    rejects(fn()=>connection_settings('instagram-xavier', ['mode'=>'api','api_key'=>'secret']));
    rejects(fn()=>connection_settings('instagram-xavier', ['client_id'=>'id','client_secret'=>"bad\nsecret"]));
    rejects(fn()=>connection_settings('youtube-ivt', ['mode'=>'api','api_key'=>'key','channel_id'=>'not-a-channel']));
    $config = connection_settings('instagram-xavier', ['client_id'=>'app-id','client_secret'=>'private-test-secret', 'extra'=>'ignored']);
    master_store_update($dir.'/providers.json', [], static fn($data)=>['instagram'=>$config, 'youtube'=>['api_key'=>'private-api-key']]);
    expect(connection_public()['instagram-xavier']['configured'], 'OAuth ready after configuration');
    expect(!connection_public()['instagram-xavier']['active'], 'settings alone do not authenticate');
